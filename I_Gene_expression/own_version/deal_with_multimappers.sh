#!/bin/bash
###############################################################################
# OWN_VERSION FORK -- differs from a_Mapping/deal_with_multimappers.sh in
# exactly TWO places; everything else is byte-identical to upstream.
# The full rationale is in the header of deal_with_singlemappers.sh; briefly:
#
# (1) Upstream selected multimappers with  $0 ~ /NH:i:[2-9]/ , which inspects a
#     single character and so misses NH:i:10 .. NH:i:19 entirely. Those
#     alignments did not match the singlemapper pattern either, so they were
#     dropped by both -- ~5% of this library's multimapping reads. The NH value
#     is now parsed as a number and tested with nh >= 2.
#
# (2) `if (readstrand="+")` is an assignment, not a comparison -- always true,
#     dead `else`, and it clobbered readstrand. Now `==`. Does not change the
#     count tables (see the other script's header), fixed because it is wrong.
###############################################################################
...
${p2samtools}/samtools view -h $inbam | awk 'BEGIN{OFS="\t"} {
    if ($1 ~ /^@/) {print $0}
        ...
        if (nh >= 2) {$1=$1";CG:"$6";nM:"nm; print $0}
    }
