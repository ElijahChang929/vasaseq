...
step_check() {
    ...
    # -- scratch (step4 writes STAR's discarded logs here) --
    if mkdir -p "$SCRATCH" 2>/dev/null && [ -w "$SCRATCH" ]; then
        echo "  OK   $SCRATCH"
    else
        echo "  MISS $SCRATCH  (not creatable/writable -- see SCRATCH in config.sh)"; bad=1
    fi
    ...
    # Test for SA, not just the directory: STAR --runMode genomeGenerate creates
    # the directory up front, so a still-building or failed index would
    # otherwise pass check and only blow up hours later in step4.
    if [ -s "${STAR_INDEX}/SA" ] && [ -s "${STAR_INDEX}/Genome" ]; then
        echo "  OK   $STAR_INDEX"
    else
        # NB: no builder script exists for this index -- build_mouse_reference.sh
        # was deleted. It has to be regenerated by hand; see config.sh.
        echo "  MISS ${STAR_INDEX}/SA  (index absent or still building; there is NO build script for it -- see the STAR_INDEX comment in config.sh)"; bad=1
    fi
...
#
# WHY BOTH ALIGNERS, measured on this library rather than taken on faith:
...
# 20.2% of this library's trimmed reads are under 30 nt. On cell 001 the
# aln-only group is 18,478 reads averaging 24.4 nt, 87% of them under 30 --
# 49% of all rRNA detection in that cell. Dropping `bwa aln` would silently
# leak every short rRNA fragment. Leave ribo-bwamem.sh alone.
#
# THE REFERENCE MATTERS MORE THAN ANYTHING ELSE HERE. RRNA_FASTA must be
# unique_rRNA_mouse.v2.fa, built by ./build_rrna_reference.sh -- v1 was
# Ensembl-derived and contained no 28S and no ETS/ITS at all, because the rDNA
# array is collapsed in the GRCm39 primary assembly. Measured against v1 on
# cell 001: 1.92% of reads called ribosomal, versus 12.14% with v2, and 82.9%
# of the recovered reads are 28S or 5'ETS. See build_rrna_reference.sh's header
# for the full argument and the specificity check (0 false positives in 20,000
# simulated protein-coding reads, under both references, at 130 nt and 50 nt).
#
# Sizing, MEASURED on the first full 16-cell run (job 50787728, NCORES=16,
# 91.3 M reads / 5.4 GB of trimmed fastq): 19m30s elapsed, TotalCPU 4h48m,
# MaxRSS 16.3 GB. That run was given -c 32, which was too much -- TotalCPU over
# Elapsed is 14.8 cores average, so 16 is the right ask.
#
# Unlike step2 the core count is not simply NCORES: a cell is one worker, but
# ribo-bwamem.sh is internally threaded (`bwa mem -t 8`, two `samtools sort
# --threads 8`, aln and mem running concurrently). It still averages under one
# core per cell because the rRNA index is tiny (71 kB) and the stage is
# dominated by gzip I/O, not alignment. The tail is long -- cells differ 60x in
# size, so the last two cells run alone for several minutes.
###############################################################################

# sbatch -c 16 --mem=24G --time=1:00:00 pipeline.sh step3
...
step3_ribo() {
    ...
    # Blanks are reported identically to real cells; that is the point of them.
    if [ -x "$STEP3_REPORT" ]; then
        echo
        env -u PYTHONPATH RRNA_FASTA="$RRNA_FASTA" "$TRIM_PYTHON" "$STEP3_REPORT" "$CELLDIR" \
            | tee "${LOGDIR}/step3_report.txt"
    fi
}
...
step4_map() {
    ...
    eval "$ML_STAR"
    # Scratch, not OUTDIR: these hold only the discarded logs from the
    # LoadAndExit / Remove calls below. See SCRATCH in config.sh.
    mkdir -p "$SCRATCH" || die "cannot create SCRATCH: $SCRATCH"
    local tmp; tmp=$(mktemp -d "${SCRATCH}/star.XXXXXX") || die "cannot write to SCRATCH: $SCRATCH"
